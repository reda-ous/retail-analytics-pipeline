...
public class EventGeneratorService {
    ...
    private enum EventKind { ORDER_CREATED, STOCK_UPDATED, PRICE_CHANGED }

    private final KafkaTemplate<String, SalesEvent> kafkaTemplate;
    private final String topic;

    public EventGeneratorService(KafkaTemplate<String, SalesEvent> kafkaTemplate,
    ...
    public void generateEvent() {
        ...
        EventKind kind = EventKind.values()[ThreadLocalRandom.current().nextInt(EventKind.values().length)];

        SalesEvent event = switch (kind) {
            case ORDER_CREATED -> new OrderCreated(
                    UUID.randomUUID(), Instant.now(), productId,
                    ThreadLocalRandom.current().nextInt(1, 10),
                    BigDecimal.valueOf(ThreadLocalRandom.current().nextDouble(5, 200)));
            case STOCK_UPDATED -> new StockUpdated(
                    UUID.randomUUID(), Instant.now(), productId,
                    ThreadLocalRandom.current().nextInt(0, 500));
            case PRICE_CHANGED -> new PriceChanged(
                    UUID.randomUUID(), Instant.now(), productId,
                    BigDecimal.valueOf(19.99), BigDecimal.valueOf(24.99));
        };
