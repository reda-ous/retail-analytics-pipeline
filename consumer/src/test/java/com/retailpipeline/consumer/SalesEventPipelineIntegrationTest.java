...
/**
 * Exercises the real pipeline end to end: a raw JSON event is published straight to a Kafka broker
 * running in a Testcontainers container (standing in for the producer), and the assertion polls the
 * REST API until the consumer — backed by a containerized Postgres — has aggregated it. Nothing
 * here is mocked.
 */
@Testcontainers
...
class SalesEventPipelineIntegrationTest {
  ...
  void orderCreatedEventIsAggregatedIntoProductStats() throws Exception {
    ...
    OrderCreated event =
        new OrderCreated(UUID.randomUUID(), Instant.now(), productId, 4, new BigDecimal("19.99"));
    ...
    await()
        ...
        .untilAsserted(
            () ->
                restTestClient
                    .get()
                    .uri("/api/stats/products/{id}", productId)
                    .exchange()
                    .expectStatus()
                    .isOk()
                    ...
                    .value(
                        stats -> {
                          ...
                          assertThat(stats.totalRevenue())
                              .isEqualByComparingTo(new BigDecimal("79.96"));
                        }));
  }
  ...
  private void publish(SalesEvent event) throws Exception {
    ...
    try (Producer<String, String> producer = new KafkaProducer<>(producerProps)) {
      producer
          .send(
              new ProducerRecord<>(
                  "sales-events", event.productId(), objectMapper.writeValueAsString(event)))
